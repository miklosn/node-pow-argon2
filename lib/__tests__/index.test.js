...
describe("verify", () => {
  it("should throw error with a really bad hash", async () => {
    try {
  ...
  it("should throw error with an unparsable hash", async () => {
    try {
      let err = await verify(
        "alma",
        "$argon2id$v=19$zzz$vTysnpkBUZ5JOw1plLzOlA$00y0PL/OgyLRCvsO+ovLjgBCrmOb+HTqXpvvc2GWZNw"
      );
    } catch (e) {
      expect(e).toBeInstanceOf(Error);
      return;
